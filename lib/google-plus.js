...
GooglePlus.prototype.userPosts = function(userId, callback) {
  var url = 'https://www.googleapis.com/plus/v1/people/' + userId + '/activities/public?' + querystring.stringify({key: this.apiKey})
  request(url, function(error, response, body) {
    ...
    if (json.error) {
      var type = function() {
        if (json.error.code >= 500) return Error
...
GooglePlus.prototype.userGoogleReaderPosts = function(userId, callback) {
  var url = 'https://www.googleapis.com/plus/v1/people/' + userId + '/activities/public?' + querystring.stringify({key: this.apiKey})
  request(url, function(error, response, body) {
    if (error) return callback(error)
    try {
    ...
    if (json.items) {
      json.items.each(function(item) {
	    if (item.provider.title == "Google Reader") {
          posts.push({
            url: urlFromAttachments(item),
            title: postTitle(item, false),
            updated: new Date(item.updated),
            author: item.actor.displayName
          })
        }
...
var urlFromAttachments = function(item) {
    if (item.object.attachments) {
        for (var i = 0; i < item.object.attachments.length; i++) {
            url = item.object.attachments[i].url
            if (url) return url
        }
    }
}

var postTitle = function(item, includeAttachmentType) {
    ...
    if (item.object.attachments && includeAttachmentType) {
        var type = (function() {
